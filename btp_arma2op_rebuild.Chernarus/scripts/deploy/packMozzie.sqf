...
if(isNull _ct || _ctType != "CSJ_GyroC") exitWith {cutText ["You can pack only built Mozzie!", "PLAIN DOWN"];};
if(_ct getVariable ["MalSar",0] != 1) exitWith {cutText ["This mozzie is completely destroyed!", "PLAIN DOWN"];};
...
if(inSafeZone) exitWith {cutText ["ANTI-TROLL: Cannot pack Mozzie in a Safe Zone!","PLAIN DOWN"];};
...
if (_finished) then {
	...
	cutText ["\n\nCanceled packing Mozzie!", "PLAIN DOWN",4];
};
