...
*/
	...
	if((_banker == 1) and (player distance _cursorTarget < 3)) then {        
        if (s_bank_dialog9 < 0) then {
            s_bank_dialog9 = player addAction ["<t color='#DD5D25'>Bank</t>", "Scripts\CustomCurrency\Bank\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
        }; 
		if (s_player_BTPcareer < 0) then {
			s_player_BTPcareer = player addAction ["<t color=""#DD5D25"">Get a Job</t>","Scripts\Jobs\jobs.sqf","",5,false,true,"",""];
		};
...
if (_typeOfCursorTarget in DZE_LockableStorage) then {	
	 if (s_player_hotwirevault < 0) then {                 	 
	     s_player_hotwirevault = player addaction [("<t color=""#B40404"">" + ("Hotwire Vault") +"</t>"), "scripts\HotWire\forcevault.sqf",_cursorTarget, 0, false, true, "", ""];
          };				   
          } else {
	...
	*/
	...
	if (_typeOfCursorTarget in DZE_MoneyStorageClasses && {!locked _cursorTarget} && {!(_typeOfCursorTarget in DZE_LockedStorage)} && {player distance _cursorTarget < 5}) then {
		if (s_bank_dialog < 0) then {
			// No object storage
			//s_bank_dialog = player addAction ["Online Banking","scripts\zsc\atmDialog.sqf",_cursorTarget,3,true,true];
		};
	} else {
	...
	if (_typeOfCursorTarget in ZSC_bankObjects && {player distance _cursorTarget < 5}) then {
		if (s_bank_dialog2 < 0) then {
			s_bank_dialog2 = player addAction ["Online Banking","scripts\zsc\atmDialog.sqf",_cursorTarget,3,true,true];
		};
	} else {
